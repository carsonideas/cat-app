...
myFactsSubmitButton.addEventListener("click", async (e) => {
  ...
  try {
    ...
    const res = await axios.get(`https://meowfacts.herokuapp.com/?count=${factsCount}`);
    const list = res.data.data.map(fact =>
...
myPhotosSubmitButton.addEventListener("click", async (e) => {
  ...
  try {
    ...
    const res = await axios.get(`https://api.thecatapi.com/v1/images/search?limit=${photosCount}`);
    myPhotosResultContainer.innerHTML = res.data.map((photo, i) =>
      `<div><img src="${photo.url}" alt="Cat ${i + 1}" style="width:100%; height:200px;"></div>`
    ).join('');
  } catch (error) {
